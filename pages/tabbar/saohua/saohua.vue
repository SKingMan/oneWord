...
<uni-section title="骚话连篇" type="line">
	...
	export default {
		...
		methods:{
				fetchcontent(){
					...
					// 如果随机数小于0.5，则调用functionA；否则调用functionB
					    if (random < 0.5) {
					        this.fetchcontent_sao();
					    } else {
					        this.fetchcontent_qing();
					    }
				},
				fetchcontent_sao(){
					this.fetchimage();
					uni.request({
							...
							success: (res) => {
							  if (res.statusCode === 200) {
								this.temp = res.data; // 假设返回的数据是一个数组
								this.jok_content = this.temp.data.content
				...
				actionsClick_share(type) {
					...
					this.messageText = `嘿嘿，涩涩的东西不能分享哦~`
					this.$refs.message.open()
				},
				actionsClick_zan(type) {
				
					this.messageText = `hahah，也不能点赞哦~`
					this.$refs.message.open()
				},
